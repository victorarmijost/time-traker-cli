...
func (kern *App) stopRecordWithDate(ctx context.Context, endTime time.Time) (float64, error) {
	...
	if !openRecord.IsEmpty(endTime) {
		...
		if domain.TodayIsWeekend() {
			err = record.SendToPool()
			if err != nil {
				return 0, fmt.Errorf("error sending record to pool, %w", err)
			}
		}
	...
	for _, record := range records {
		if remTime < record.Hours() {
			...
			if math.Round(remTime*10000) == 0 {
				...
				err = record.UpdateHours(remTime)
				if err != nil {
					return err
				}
...
func (kern *App) GetDebt(ctx context.Context) (*domain.Debt, error) {
	return kern.stats.GetDebt(ctx, kern.config.GetWorkTime())
}
